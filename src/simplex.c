...
struct grad grad3[12];
struct grad grad4[32];
...
short perm[512];
short perm_mod_12[512];

void simplex_init() {
  grad3[ 0] = create_grad( 1, 1, 0, 0); grad3[ 1] = create_grad(-1, 1, 0, 0); grad3[ 2] = create_grad( 1,-1, 0, 0);
  grad3[ 3] = create_grad(-1,-1, 0, 0), grad3[ 4] = create_grad( 1, 0, 1, 0); grad3[ 5] = create_grad(-1, 0, 1, 0);
  grad3[ 6] = create_grad( 1, 0,-1, 0); grad3[ 7] = create_grad(-1, 0,-1, 0); grad3[ 8] = create_grad( 0, 1, 1, 0);
  grad3[ 9] = create_grad( 0,-1, 1, 0); grad3[10] = create_grad( 0, 1,-1, 0); grad3[11] = create_grad( 0,-1,-1, 0);

  grad4[ 0] = create_grad( 0, 1, 1, 1); grad4[ 1] = create_grad( 0, 1, 1,-1); grad4[ 2] = create_grad( 0, 1,-1, 1);
  grad4[ 3] = create_grad( 0, 1,-1,-1); grad4[ 4] = create_grad( 0,-1, 1, 1); grad4[ 5] = create_grad( 0,-1, 1,-1);
  grad4[ 6] = create_grad( 0,-1,-1, 1); grad4[ 7] = create_grad( 0,-1,-1,-1); grad4[ 8] = create_grad( 1, 0, 1, 1);
  grad4[ 9] = create_grad( 1, 0, 1,-1); grad4[10] = create_grad( 1, 0,-1, 1); grad4[11] = create_grad( 1, 0,-1,-1);
  grad4[12] = create_grad(-1, 0, 1, 1); grad4[13] = create_grad(-1, 0, 1,-1); grad4[14] = create_grad(-1, 0,-1, 1);
  grad4[15] = create_grad(-1, 0,-1,-1); grad4[16] = create_grad( 1, 1, 0, 1); grad4[17] = create_grad( 1, 1, 0,-1);
  grad4[18] = create_grad( 1,-1, 0, 1); grad4[19] = create_grad( 1,-1, 0,-1); grad4[20] = create_grad(-1, 1, 0, 1);
  grad4[21] = create_grad(-1, 1, 0,-1); grad4[22] = create_grad(-1,-1, 0, 1); grad4[23] = create_grad(-1,-1, 0,-1);
  grad4[24] = create_grad( 1, 1, 1, 0); grad4[25] = create_grad( 1, 1,-1, 0); grad4[26] = create_grad( 1,-1, 1, 0);
...
const double F3 = 0.33333333333; /* 1.0/3.0 */
const double G3 = 0.16666666666; /* 1.0/6.0; */
...
double dot3d(struct grad* g, double x, double y, double z) {
  return g->x * x + g->y * y + g->z * z;
}

double dot4d(struct grad* g, double x, double y, double z, double w) {
